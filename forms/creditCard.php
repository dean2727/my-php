...
// Write your code here:
    if (strlen($card_num) < 100) {
      if ($card_type === "mastercard") {
        $feedback = (preg_match("/5[1-5][0-9]{14}/", $card_num) === 1) ? $success_message : $error_message;
      }
      else {
        $feedback = (preg_match("/4[0-9]{12}([0-9]{3})?([0-9]{3})?/", $card_num) === 1) ? $success_message : $error_message;
      }
    }
    else {
